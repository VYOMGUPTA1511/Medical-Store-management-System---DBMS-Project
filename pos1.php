...
<body>
		...
		<div class="sidenav">
			...
			<p style="margin-top:-20px;color:white;line-height:1;font-size:12px;text-align:center">Vyom's, 2024</p>
	...
	?>
	 ...
	 <div class="one row" style="margin-right:160px;">
        <form method="post" action="pos1.php">
            <div class="column">
                <label for="medid">Medicine ID:</label>
            ...
            <div class="column">
                <label for="mcat">Category:</label>
                <input type="text" name="mcat" value="<?php echo isset($row4[3]) ? $row4[3] : ''; ?>"><br><br>

                <label for="mloc">Location:</label>
                <input type="text" name="mloc" value="<?php echo isset($row4[5]) ? $row4[5] : ''; ?>"><br><br>
            </div>
            <div class="column">
                <label for="mqty">Quantity Available:</label>
                <input type="number" name="mqty" value="<?php echo isset($row4[2]) ? $row4[2] : ''; ?>"><br><br>

                <label for="mprice">Price of One Unit:</label>
                <input type="number" name="mprice" value="<?php echo isset($row4[4]) ? $row4[4] : ''; ?>"><br><br>
            </div>

            <label for="mcqty">Quantity Required:</label>
            <input type="number" name="mcqty"><br><br>
            <input type="submit" name="add" value="Add Medicine">
		<?php
		...
		if (isset($_POST['add'])) {
			...
			if ($qty > $aqty || $qty == 0) {
				...
				$checkQry = "SELECT * FROM sales_items WHERE sale_id = $sid AND med_id = $mid";
				$checkResult = $conn->query($checkQry);
		
				if ($checkResult->num_rows > 0) {
					// Record exists, update the quantity and total price
					$updateQry = "UPDATE sales_items 
								  SET sale_qty = sale_qty + $qty, 
									  tot_price = tot_price + $price 
								  WHERE sale_id = $sid AND med_id = $mid";
					$conn->query($updateQry);
					echo "Record updated successfully.";
				} else {
					// Record doesn't exist, insert a new one
					$insertQry = "INSERT INTO sales_items (sale_id, med_id, sale_qty, tot_price) 
